 ...
 *
 * Steps arrive as \`data\` parts named \`step\`; clicking one opens this panel
 * with its arguments, result, and timeline.
 */
...
export function AgUiInspector() {
    ...
    for (const message of messages) {
      ...
      for (const part of message.content) {
        if (part.type === "data" && part.name === "step") {
          ...
          if (data?.callId === inspectedCallId) return data;
        }
      }
    }
    return null;
  }, [messages, inspectedCallId]);

  // A step can disappear (thread switch / history reload): close, don't linger.
  useEffect(() => {
    if (inspectedCallId && !step) close();
  }, [inspectedCallId, step, close]);

  useEffect(() => {
    if (!inspectedCallId || !step) return undefined;
  ...
  if (!inspectedCallId || !step) return null;
