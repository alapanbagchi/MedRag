...
export function useAgUiPlan(): ComposerPlanData | null {
 ...
 // Same reason as the sub-agent view model: this folds every message on each
 // streamed token. Deferring keeps the token stream urgent and coalesces the
 // fold, so a long run no longer re-derives the whole plan hundreds of times.
 const deferred = useDeferredValue(messages);
 return useMemo(
 () => deriveComposerPlan(deferred as unknown as MessageLike[]),
 [deferred],
 );
}
...
export function PlanMorph({
 ...
 <div
 className={cn(
 "w-full overflow-hidden rounded-[26px] border border-border bg-card backdrop-blur-2xl transition-[border-radius,box-shadow] duration-500 ease-out focus-within:border-brand/50 focus-within:ring-2 focus-within:ring-brand/20",
...
export function ComposerPlan({ plan }: { plan: ComposerPlanData }) {
 ...
 <ListChecksIcon className="size-4 shrink-0 text-brand" />
 ...
 className="ml-auto flex size-7 shrink-0 items-center justify-center rounded-full text-muted-foreground outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-brand/40"
 >
 <ChevronDownIcon
...
function TaskRow({ task, last }: { task: PlanTaskItem; last: boolean }) {
 ...
 >
 <span className={cn(mono, "mr-1.5 text-[11.5px] font-normal text-brand/60")}>
 {task.id}
 </span>
 {task.text}
 </span>
...
function EvidenceRow({ evidence, last }: { evidence: PlanEvidence; last: boolean }) {
 ...
 >
 <span className={cn(mono, "mr-1.5 text-[10.5px] text-brand/70")}>
 {evidence.id}
 </span>
 {evidence.text}
 </span>
...
function EvidenceGlyph({ status }: { status: EvidenceStatus }) {
 if (status === "satisfied") return <CheckIcon className="size-4 text-success" />;
 if (status === "partial") return <TriangleAlertIcon className="size-3.5 text-warning" />;
 if (status === "unsatisfied") return <XIcon className="size-3.5 text-danger" />;
...
function StatusDot({ status }: { status: TaskStatus }) {
 ...
 <span aria-hidden className="absolute inline-flex size-2.5 animate-ping rounded-full bg-brand/35 motion-reduce:hidden" />
 <span aria-hidden className="relative size-2.5 rounded-full bg-brand" />
 </span>
 );
 }
 const tone =
