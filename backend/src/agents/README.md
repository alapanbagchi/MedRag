# src.agents - the agentic retrieval system (deepagents + LangGraph)

The **singular** research pipeline, living inside backend/src so it shares the
...
the single boundary in agents/reuse.py. All agents, state, graph,
...
- [x] Stage agents construct (orchestrator, planner, verifier, ...)
- [x] State models (src/agents/state.py) - evidence/requirement/run
      state machines with explicit transitions
- [x] Workflow rules (src/agents/rules.py) - the mandatory gates:
...
- [x] Tool adapters (src/agents/tools/) - langchain @tools wrapping
...
- [x] Real research graph (src/agents/graph.py): decompose ->
...
uv run pytest tests/test_agents_* -q
uv run python -m src.agents "<question>" --run
```
...
The MedPat frontend talks to this system through `/v1/chat/stream` - the
singular flow speaks the wire contract (status / sources / token / done /
pipeline / memory trace). To drive the UI:
...
Every request runs the agents graph. Every run still logs to
backend/logs.txt ([xdeep] events) and streams live status into the UI's
