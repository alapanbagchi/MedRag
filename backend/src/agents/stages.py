"""Stage agents for the autonomous research runtime (deepagents).

  PLANNER (planner.py, master.txt prompt)
    -> per-task WORKER loop (tool-choosing worker):
         ...
         VERIFIER critic (critic.txt, verify lane) - HARD gate
  ...
  CONTRADICTION (contradiction.txt, think lane)
  RESOLUTION (resolution.txt, think lane + search tools)
  SYNTHESIS (synthesize.txt, gemma)

The stage order (conflict -> resolution -> gap resolution -> synthesis) is
driven by deep.py; within research the worker LLM decides HOW (which tools,
which queries, whether to inspect deeper).
"""
...
from src.agents.tracing import traced_ainvoke
...
async def plan_queries(agent: Any, task: str, requirement_text: str,
    ...
    result = await traced_ainvoke(agent, 
...
# ---------------------------------------------------------------------------
# TOOL SELECTOR (tool_selector.txt) - the worker's per-step dispatcher.
# The agent picks ONE tool per step (or finish); code executes it and
# enforces that tool's guardrail. No fixed order - corpus before web is a
# prompted preference, not a code path.
# ---------------------------------------------------------------------------

WORKER_TOOL_SCOPE = (
    "postgres_search", "retrieve", "searxng_search", "umls_lookup",
    "inspect_paper", "lookup_by_author", "deep_inspect", "finish",
)

_TOOL_CHOICE_SCHEMA = (
    "{\"tool\":\"postgres_search|retrieve|searxng_search|umls_lookup|"
    "inspect_paper|lookup_by_author|deep_inspect|finish\","
    "\"query\":\"<natural-language query or ''>\","
    "\"args\":{},\"finish\":false,\"reason\":\"<one line>\"}"
)


def make_tool_selector(model: Any = None):
    """Per-step tool dispatcher: tool_selector.txt on the research lane."""
    try:
        base = load_prompt("agents", "tool_selector.txt")
    except Exception:
        base = "You are a medical research tool selector."
    return create_deep_agent(
        model=model or build_model_for_role("research"),
        system_prompt=base,
        tools=[],
...
def tool_choice_prompt(task: str, requirement_text: str, target_n: int,
                       verified_n: int, tried: list, critic_notes: str,
                       searches_left: int, steps_left: int,
                       scope: list[str], args: dict) -> str:
    lines = [
        f"TASK: {task}",
        f"REQUIREMENT: {requirement_text}",
        f"TARGET: {target_n} independent verified documents, "
        f"VERIFIED SO FAR: {verified_n}",
        f"ALLOWED TOOLS THIS SUBTASK: {', '.join(scope)}",
        f"ALREADY TRIED: {', '.join(tried) if tried else '(none)'}",
        f"SEARCHES LEFT: {searches_left}, STEPS LEFT: {steps_left}",
    ]
    if args:
        lines.append(f"TASK ARGS (ids the task carries): {args}")
    if critic_notes:
        lines.append(f"CRITIC NOTES ON REJECTED CANDIDATES:\n{critic_notes}")
    return _prompt("\n".join(lines), "", _TOOL_CHOICE_SCHEMA)


async def choose_tool_action(agent: Any, task: str, requirement_text: str,
                             target_n: int, verified_n: int,
                             tried: list | None = None,
                             critic_notes: str = "",
                             searches_left: int = 0, steps_left: int = 1,
                             scope: list[str] | None = None,
                             args: dict | None = None) -> dict:
    """One dispatcher call -> sanitized action dict.

    Never raises: unparsable output becomes {"tool": "__default__"} and the
    worker maps it to the first in-scope tool (or finish when scope is
    empty), so a confused selector degrades to a deterministic default.
    """
    scope = [t for t in (scope or []) if t in WORKER_TOOL_SCOPE]
    try:
        result = await traced_ainvoke(agent, 
            {"messages": [{"role": "user", "content": tool_choice_prompt(
                task, requirement_text, target_n, verified_n,
                tried or [], critic_notes or "", searches_left,
                steps_left, scope, args or {})}]}
        )
        data = _extract_json(_last_text(result.get("messages", []))) or {}
    except Exception:
        data = {}
    ...
    if tool not in scope and tool != "finish":
        tool = "__default__"
    query = _sanitize_semantic_query(str(data.get("query", "") or ""))
    return {"tool": tool,
            "query": query or requirement_text,
...
def make_verifier(model: Any = None):
    """Verifier critic: critic.txt on the verify lane (Gemini Gemma 4 31B)."""
    return create_deep_agent(
...
def make_reliability_critic(model: Any = None):
    """Reliability critic: reliability.txt on the verify lane (Gemini Gemma 4 31B).
...
async def fetch_web_hit(url: str, snippet: str, requirement_id: str,
                        *, tag: str, extra: dict | None = None) -> tuple:
    """Fetch one web hit's real page bundle (snippet fallback).
    ...
    from src.agents.tools.fetch import fetch_page_bundle

    snippet = (snippet or "").strip()
    _bundle: dict = {"text": "", "fetched_ok": False, "page_title": "",
                      "page_header": "", "favicon_url": "", "fetched_chars": 0}
    try:
        _raw = await run_with_timeout(
            fetch_page_bundle(url or "", max_chars=8000),
            timeout_for("llm_call"), f"{tag}-fetch:{str(url or '')[:60]}")
    except Exception:
        _raw = {}
    # backwards-compat: tests/mocks may return a plain text string
    if isinstance(_raw, str):
        _bundle = {"text": _raw, "fetched_ok": bool(_raw),
                   "page_title": "", "page_header": "",
                   "favicon_url": "", "fetched_chars": len(_raw)}
    elif isinstance(_raw, dict) and _raw:
        _bundle = _raw
    page_text = (_bundle.get("text", "") or "")
    text = (page_text or snippet).strip()
    if len(text) < 40:
        return "", False, _bundle
    emit_event("web_fetch", requirement_id=requirement_id, url=url or "",
               ...
               page_title=str(_bundle.get("page_title", ""))[:120],
               **(extra or {}))
    if page_text:
        _p(f"        [web-fetch:{requirement_id}] {str(url or '')[:80]} "
           f"({len(page_text)} chars)")
    return text, bool(page_text), _bundle
...
async def verify_web_hit(req: Any, item: Any, *, verifier: Any,
    ...
    try:
        ...
        await run_with_timeout(
            submit_verify(verifier, req, item),
...
def stamp_web_provenance(item: Any, hit: dict, bundle: dict, query: str) -> Any:
    from src.agents.state import domain_of
    from src.agents.tools.site_reputation import classify_site_tier

    url = hit.get("url", "") or getattr(item, "source_url", "") or ""
    item.source_kind = "web"
    item.source_url = url
    item.domain = domain_of(url)
    try:
        item.site_tier = hit.get("site_tier") or classify_site_tier(url)
    except Exception:
        item.site_tier = hit.get("site_tier", "") or ""
    item.trust = hit.get("trust", "") or getattr(item, "trust", "") or "unverified"
    item.page_title = (bundle.get("page_title", "") or "")[:300]
    item.page_header = (bundle.get("page_header", "") or "")[:500]
    item.favicon_url = bundle.get("favicon_url", "") or ""
    item.snippet = (hit.get("snippet", "") or "")[:500]
    item.fetch_ok = bool(bundle.get("fetched_ok", False))
    try:
        item.fetched_chars = int(bundle.get("fetched_chars", 0) or 0)
    except Exception:
        item.fetched_chars = 0
    item.engine = str(hit.get("engine", "") or "searxng")
    if query:
        item.source_query = query
    if not item.site_name:
...
def make_contradiction_agent(model: Any = None):
    """Contradiction/anomaly agent: contradiction.txt on the think lane."""
    return create_deep_agent(
...
def make_resolution_agent(model: Any = None):
    """Contradiction resolver: resolution.txt on the think lane WITH search
...
async def resolve_contradiction(agent: Any, verifier: Any, contradiction: Any,
    ...
    for qi, query in enumerate(queries):
        ...
        for ci, cand in enumerate(candidates):
            ...
            is_web = bool(cand.get("source_url", ""))
            item.source_kind = "web" if is_web else "corpus"
            if is_web:
                from src.agents.state import domain_of
                item.domain = domain_of(cand.get("source_url", ""))
                item.site_name = item.domain
                item.snippet = (cand.get("text", "") or "")[:500]
                item.engine = "searxng"
            req.add_item(item)
            try:
                from src.agents.verify_queue import submit_verify
                await submit_verify(verifier, req, item)
            except Exception as exc:
...
async def synthesize_answer_with_data(agent: Any, state: Any) -> tuple[str, dict | None]:
    ...
    """
    from src.agents.stages import fallback_synthesize as _fb
    from src.agents.logging import xdeep_log
...
__all__ = [
    "WORKER_TOOL_SCOPE", "choose_tool_action", "tool_choice_prompt",
    "make_tool_selector", "plan_queries", "replan_queries",
    ...
    "make_search_planner", "make_replanner",
