...
KNOWN_TOOLS = frozenset({"retrieve", "umls_lookup", "paper_inspect", "postgres_search"})
TOOLS_AVAILABLE_NOW = frozenset({"retrieve", "umls_lookup", "paper_inspect", "postgres_search"})
...
def phase_ordering():
    return [p.value for p in Phase.ordered()]


def legal_next(phase: Phase) -> list[Phase]:
    """Phases reachable directly from `phase` on the mandatory spine."""
    idx = phase_index(phase)
    if idx + 1 >= len(Phase.ordered()):
        return []
    return [Phase.ordered()[idx + 1]]
...
def contradiction_input_ready(state: XDeepRunState) -> bool:
    ...
    for req in state.requirements:
        if req.status not in ("satisfied", "exhausted"):
            return False
    return True
