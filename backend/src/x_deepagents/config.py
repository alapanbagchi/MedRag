...
from src.x_deepagents.reuse import AppConfig
...
def think_provider() -> str:
    ...
    Resolution order:
      1. XDEEP_THINK_PROVIDER forced: qwen | alibaba | aliyun | groq | google
...
def build_think_model():
    ...
    or groq/google when forced / env-selected):
...
def build_gemma_model():
    """Backward-compatible alias: 'good agent' (now Qwen-or-Groq-or-Google)."""
    return build_think_model()
