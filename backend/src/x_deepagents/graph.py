...
from src.x_deepagents import rules
...
async def decompose_node(state: ResearchState) -> Any:
    ...
    try:
        ...
        reqs = await run_with_timeout(
            decompose_requirements(make_orchestrator(), question),
...
def _workers_semaphore():
    ...
    if _worker_semaphore is None:
        from src.x_deepagents.timeouts import max_workers
...
async def research_worker(payload: TaskPayload) -> dict:
    ...
    """
    from src.x_deepagents.agents.worker import research_worker as _rw
    from src.x_deepagents.timeouts import run_with_timeout, timeout_for
    ...
    for c in run.contradictions:
        try:
            ...
            await run_with_timeout(
                resolve_contradiction(resolver, verifier, c),
            ...
            from src.x_deepagents.state import ResolutionOutcome, ResolutionStatus
        ...
        try:
            ...
            if fresh:
                ...
                for fc in fresh:
                    try:
                        await run_with_timeout(
                            resolve_contradiction(resolver, verifier, fc),
...
async def run_research(question: str, graph=None) -> XDeepRunState:
    ...
    way as the CLI.
    """
    from src.x_deepagents.logging import close_run_log, open_run_log
