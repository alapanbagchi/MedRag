...
# process. Default: logs.txt next to the CWD (backend/logs.txt when run from
# the backend dir, matching the legacy MEDRAG log).
_OPEN = False


def log_path() -> Path:
    return Path(os.environ.get("XDEEP_LOG_FILE", "logs.txt"))
