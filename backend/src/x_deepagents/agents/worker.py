"""standalone cell - spliced into graph.py by _splice_graph.py"""

import json
import logging

from src.x_deepagents.graph import emit_event
from src.x_deepagents.state import EvidenceItem, ResearchRequirement, RunBudget
from src.retrieval.plans import PlannedEntity, SubQueryPlan
from src.x_deepagents.timeouts import run_with_timeout, timeout_for
...
logger = logging.getLogger("x_deepagents.graph.splice")
...
async def _local_retrieve(payload: dict, query: str, seen_chunks: set, top_k: int = 5):
    ...
    """
    from src.x_deepagents.tools.postgres_search import postgres_search_impl
    ...
    if not data.get("available"):
        from src.x_deepagents.graph import progress
    ...
    from src.x_deepagents.reuse import get_hybrid_retriever
...
async def _web_fill(payload: dict, req: ResearchRequirement, queries: list[str],
    ...
    from src.x_deepagents.agents.stages import verify_item
    from src.x_deepagents.graph import progress
    from src.x_deepagents.tools.searxng import searxng_base_url, searxng_search_impl
    ...
    for query in queries:
        ...
        for i, hit in enumerate((data.get("results") or [])[:5], 1):
            ...
            from src.x_deepagents.tools.fetch import fetch_page_text
            from src.x_deepagents.graph import progress as _p

            snippet = (hit.get("snippet") or "").strip()
            page_text = await _llm(
                fetch_page_text(hit.get("url", ""), max_chars=8000),
                f"web-fetch:{str(hit.get('url',''))[:60]}")
            text = (page_text or snippet).strip()
            if len(text) < 40:
                continue
            if page_text:
                _p(f"        [web-fetch:{task_id}] {str(hit.get('url',''))[:80]} "
                   f"({len(page_text)} chars)")
                emit_event("web_fetch", requirement_id=task_id,
                           url=hit.get("url", ""), chars=len(page_text),
                           ok=True, query=query)
            else:
                emit_event("web_fetch", requirement_id=task_id,
                           url=hit.get("url", ""), chars=0, ok=False,
                           snippet_used=True, query=query)
            item = EvidenceItem(
            ...
            req.add_item(item)
            try:
                await _llm(verify_item(verifier, req, item), f"web-verify:{item.id}")
            except Exception as exc:
                logger.warning("web verification failed for %s: %s", item.id, exc)
            ...
            if item.verified and item.source_url:
                try:
                    verdict = await _llm(
                        judge_site_reliability(reliability_critic, item.source_url, item.text[:4000]),
                        f"reliability:{item.id}")
                except Exception as exc:
                    logger.warning("reliability judge failed for %s: %s", item.id, exc)
                    item.reliability = "low"
                    continue
                item.reliability = verdict["reliability"]
                progress(f"            [reliability] {item.id} -> {verdict['reliability']} "
                         f"({item.source_url[:80]})")
                emit_event("reliability_verdict", requirement_id=task_id,
                           evidence_id=item.id,
                           url=item.source_url[:160],
