...
from src.x_deepagents.config import build_model_for_role
...
async def _decompose_call(agent: Any, question: str, strict: bool = False) -> list[DecomposedReq]:
...
async def decompose_requirements(agent: Any, question: str) -> list[DecomposedReq]:
...
def make_search_planner(model: Any = None):
    ...
    from src.x_deepagents.tools.umls import umls_lookup
...
async def verify_item(agent: Any, requirement: Any, item: Any) -> Any:
    ...
    from src.x_deepagents.state import VerifierVerdict
...
async def detect_contradictions(agent: Any, items: list[Any]) -> list:
    from src.x_deepagents.logging import xdeep_log
    from src.x_deepagents.state import Contradiction, ContradictionKind
...
def _log_warning(msg: str, *args) -> None:
    ...
    logging.getLogger("x_deepagents.resolution").warning(msg, *args)


async def resolve_contradiction(agent: Any, verifier: Any, contradiction: Any) -> Any:
...
def _synthesize_prompt(state) -> str:
    from src.x_deepagents.rules import verified_evidence_items
...
async def synthesize_answer(agent: Any, state: Any) -> str:
    """Synthesize and return the rendered prose answer only."""
    prose, _ = await synthesize_answer_with_data(agent, state)
    return prose
...
async def synthesize_answer_with_data(agent: Any, state: Any) -> tuple[str, dict | None]:
    ...
    """
    from src.x_deepagents.agents.stages import fallback_synthesize as _fb
    from src.x_deepagents.logging import xdeep_log
