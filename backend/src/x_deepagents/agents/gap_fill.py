...
async def _plan_gap_queries(planner: Any, replanner: Any, req: ResearchRequirement,
    ...
    from src.x_deepagents.agents.worker import _critic_notes
...
async def _web_round(req: ResearchRequirement, queries: list[str],
    ...
    from src.x_deepagents.graph import progress
    from src.x_deepagents.tools.searxng import searxng_search_impl
    from src.x_deepagents.timeouts import run_with_timeout, timeout_for
    ...
    for query in queries:
        ...
        for i, hit in enumerate((data.get("results") or [])[:5], 1):
            ...
            if item.verified and item.source_url:
                ...
                slug = _re.sub(r"[^a-z0-9]+", "-", item.source_url.lower())[:80]
                item.document_id = "web:" + slug if slug else "web:" + item.id
                try:
...
async def _probe_latent_gaps(agent: Any, req: ResearchRequirement) -> list[dict]:
    ...
    from src.x_deepagents.reuse import load_prompt
    from src.x_deepagents.timeouts import run_with_timeout, timeout_for
    ...
    for query in gap["queries"]:
        ...
        for i, hit in enumerate((data.get("results") or [])[:5], 1):
            ...
            page_text = await run_with_timeout(
                ...
                timeout_for("llm_call"), f"probe-fetch:{str(hit.get('url',''))[:60]}")
            text = (page_text or snippet).strip()
            if len(text) < 40:
                continue
            emit_event("web_fetch", requirement_id=req.id,
                       url=hit.get("url", ""),
                       chars=len(page_text) if page_text else 0,
                       ok=bool(page_text), snippet_used=not page_text,
                       purpose=gap["question"][:160])
            if page_text:
                _p(f"        [web-fetch:{req.id}] {str(hit.get('url',''))[:80]} "
            ...
            try:
                await run_with_timeout(
                    verify_item(verifier, req, item),
                    timeout_for("llm_call"), f"probe-web-verify:{item.id}")
            except Exception as exc:
                logger.warning("probe web verify failed %s: %s", item.id, exc)
            if item.verified and item.source_url:
                import re as _re
                ...
                try:
                    verdict = await run_with_timeout(
                        ...
                        timeout_for("llm_call"), f"probe-rel:{item.id}")
                except Exception as exc:
                    verdict = {"reliability": "low"}
                item.reliability = verdict["reliability"]
                ...
                if item.reliability in ("high", "medium"):
                    new_ids.append(item.id)
...
async def _web_augment(req: ResearchRequirement, queries: list[str],
    ...
    for query in queries:
        ...
        for i, hit in enumerate((data.get("results") or [])[:5], 1):
            ...
            page_text = await run_with_timeout(
                fetch_page_text(hit.get("url", ""), max_chars=8000),
                timeout_for("llm_call"), f"web-aug-fetch:{str(hit.get('url',''))[:60]}")
            text = (page_text or snippet).strip()
            if len(text) < 40:
                continue
            emit_event("web_fetch", requirement_id=req.id,
            ...
            try:
                await run_with_timeout(
                    verify_item(verifier, req, item),
                    timeout_for("llm_call"), f"web-aug-verify:{item.id}")
            except Exception as exc:
                logger.warning("web augment verify failed %s: %s", item.id, exc)
            if item.verified and item.source_url:
                import re as _re
                ...
                try:
                    verdict = await run_with_timeout(
                        judge_site_reliability(
                            reliability_critic, item.source_url, item.text[:4000]),
                        timeout_for("llm_call"), f"web-aug-rel:{item.id}")
                except Exception as exc:
                    verdict = {"reliability": "low"}
                item.reliability = verdict["reliability"]
                emit_event("reliability_verdict", requirement_id=req.id,
...
async def resolve_requirement_gaps(state: Any) -> list[GapResolution]:
    ...
    if probe_enabled():
        from src.x_deepagents.agents.stages import make_gap_probe
    ...
    if web_augment_enabled():
        for req in state.requirements:
            ...
            if not aug_queries:
                continue
            new_ids = await _web_augment(req, aug_queries[:2], verifier, budget)
            if new_ids:
