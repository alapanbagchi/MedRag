...
def test_unknown_role_rejected():
    from src.agents.config import role_model_name
...
def test_mistral_model_is_paced_base_chat_model():
    ...
    from src.agents.config import MISTRAL_MIN_INTERVAL_S, build_mistral_model
...
def test_think_model_builds(monkeypatch):
    from src.agents.config import build_think_model
...
def test_think_provider_auto_selects_groq_when_key_present(monkeypatch):
    from src.agents.config import think_provider
...
def test_qwen_defaults_model_from_general_llm(monkeypatch):
    ...
    from src.agents.config import QWEN_MODEL_FALLBACK, build_think_model
...
def test_agents_build_with_routed_models():
    ...
    for maker in (make_orchestrator, make_search_planner, make_replanner,
                  make_gap_probe, make_verifier, make_deep_inspector,
...
def test_tpm_bucket_acquire_respects_window():
    ...
    from src.agents.ratelimit import TPMBucket, reset_tpm_bucket
...
def test_tpm_bucket_oversized_request_admitted():
    ...
    from src.agents.ratelimit import TPMBucket
...
def test_groq_think_model_is_tpm_limited(monkeypatch):
    ...
    from src.agents.config import build_think_model
    from src.agents.ratelimit import TPMLimitedChatOpenAI, reset_tpm_bucket
...
def test_groq_default_model_is_gemma(monkeypatch):
    from src.agents.config import GROQ_MODEL_DEFAULT
