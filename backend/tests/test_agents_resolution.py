...
def test_resolution_runs_search_and_uses_verified_evidence(monkeypatch):
    ...
    from src.agents.agents.stages import resolve_contradiction
    ...
    monkeypatch.setattr("src.agents.agents.stages._resolution_candidates",
                        fake_candidates)
    # note: _resolution_candidates is what runs the tools; fake it to avoid DB

    monkeypatch.setattr("src.agents.agents.stages.verify_item", _verifier)
    agent = FakeAgent()
