...
# --- API singular flow (every request runs the agents graph) --------

def test_chat_stream_runs_singular_flow_for_any_engine_value(monkeypatch):
    import json
    ...
    monkeypatch.setattr(bridge_module, "_get_memory_api", lambda: None)

    client = TestClient(api_module.app)
    for body in ({"question": "q"},
                 {"question": "q", "engine": "v3"},
                 {"question": "q", "engine": "xdeep"}):
        with client.stream("POST", "/v1/chat/stream", json=body) as resp:
            assert resp.status_code == 200
            types = {json.loads(raw)["type"] for raw in resp.iter_lines()
                     if raw.strip()}
        assert "done" in types
    # all three requests ran the same singular research flow
    assert calls == ["q", "q", "q"]


def test_bridge_stage_mapping():
    from src.agents.bridge import stage_for_progress
...
def test_bridge_cites_nearest_excerpt():
    from src.agents.bridge import _cited_text
...
def test_bridge_streams_structured_events(monkeypatch):
    ...
    async def fake_run_research(question, memory_context: str = ""):
    ...
    monkeypatch.setattr("src.agents.bridge.run_research", fake_run_research)
