...
def test_local_retrieve_prefers_postgres(monkeypatch):
    ...
    from src.agents.agents import worker as worker_mod
    ...
    monkeypatch.setattr("src.agents.tools.postgres_search.postgres_search_impl", fake_pg)
...
def test_local_retrieve_falls_back_when_pg_down(monkeypatch):
    ...
    monkeypatch.setattr("src.agents.tools.postgres_search.postgres_search_impl", fake_pg)
    monkeypatch.setattr("src.agents.reuse.get_hybrid_retriever",
...
async def _accepting_verifier(agent, requirement, item):
    from src.agents.state import AnswersTask, SupportDirection, VerifierVerdict
...
def test_web_fill_called_only_when_unsatisfied(monkeypatch):
    ...
    from src.agents.agents import worker as worker_mod
    from src.agents.state import ResearchRequirement, RunBudget
    ...
    monkeypatch.setattr("src.agents.tools.searxng.searxng_search_impl", fake_searxng)
    monkeypatch.setattr("src.agents.agents.stages.verify_item", _accepting_verifier)
    monkeypatch.setattr("src.agents.agents.stages.judge_site_reliability", _fake_reliability)
...
def test_research_worker_web_only_after_local_and_returns_budget(monkeypatch):
    ...
    monkeypatch.setattr("src.agents.agents.stages.plan_queries", fake_plan)
    monkeypatch.setattr("src.agents.agents.stages.replan_queries", fake_replan)
    monkeypatch.setattr("src.agents.agents.worker._local_retrieve", fake_retrieve)
    monkeypatch.setattr("src.agents.tools.searxng.searxng_search_impl", fake_web_impl)
    monkeypatch.setattr("src.agents.agents.stages.verify_item", _accepting_verifier)
    monkeypatch.setattr("src.agents.agents.stages.deep_inspect", fake_deep_inspect)
    monkeypatch.setattr("src.agents.agents.stages.judge_site_reliability", _fake_reliability)
