...
from medrag.classification import classify_section_title
from medrag.models import CHUNK_VERSION, Block, Chunk, Document, Section
...
class ASTChunker:
    ...
    def __init__(
        ...
        # Delegate prose splitting to LangChain's recursive text splitter
        # (sentence-aware separators, no mid-word breaks).
        self._prose_splitter = RecursiveCharacterTextSplitter(
            chunk_size=self.max_prose_chars,
            chunk_overlap=0,
            separators=["\n\n", "\n", ". ", "? ", "! ", "; ", " "],
        )
    ...
    def _make_chunk(
        ...
        *,
        ...
        """Build a chunk with the shared structural fields filled in.

        Every chunk type repeats the same section/subsection/breadcrumb and
        document-metadata plumbing; this factory keeps that in one place so
        builders only pass what is specific to their type.
        """
    ...
    def _build_prose_chunk(
        ...
        chunk_id, position = self._next_prose_id(document_id)

        return self._make_chunk(
            chunk_id=chunk_id,
            text=text,
            embedding_text=self._build_embedding_text(text=text, breadcrumb=breadcrumb),
    ...
    def _build_list_chunks(
        ...
        return [
            self._make_chunk(
                ...
                document_metadata=document_metadata,
                position=position,
                source_block_ids=self._single_block_id(block),
                citation_refs=self._get_refs(block, "citation_refs"),
                footnote_refs=self._get_refs(block, "footnote_refs"),
                extra_metadata={"list_label": label if label else None},
            )
        ]
    ...
    def _build_table_chunks(
        ...
        summary_position = self._next_position()

        summary_chunk = self._make_chunk(
            chunk_id=summary_id,
            text=summary_text,
            embedding_text=self._build_embedding_text(
                text=summary_text, breadcrumb=breadcrumb, prefix=display_label
            ),
    ...
    def _build_table_row_chunk(
        ...
        return self._make_chunk(
            ...
            embedding_text=self._build_embedding_text(
                text=row_text,
                breadcrumb=breadcrumb,
                prefix=table_label if table_label else f"Table {table_id}",
            ),
            ...
            object_id=table_id,
            table_id=table_id,
            row_label=row_label if row_label else None,
            group_path=list(group_path or []),
            source_block_ids=list(source_block_ids),
            extra_metadata={"table_id": table_id, "row_index": row_index},
        )
    ...
    def _build_table_footnotes_chunk(
        ...
        return self._make_chunk(
            ...
            embedding_text=self._build_embedding_text(
                text=text,
                breadcrumb=breadcrumb,
                prefix=f"{table_label} footnotes" if table_label else f"Table {table_id} footnotes",
            ),
            ...
            object_id=table_id,
            table_id=table_id,
            source_block_ids=self._single_block_id(source_block),
            citation_refs=self._get_refs(source_block, "citation_refs"),
            footnote_refs=self._get_refs(source_block, "footnote_refs"),
    ...
    def _build_figure_chunk(
        ...
        return self._make_chunk(
            ...
            text=text,
            embedding_text=self._build_embedding_text(
                text=text, breadcrumb=breadcrumb, prefix=f"Figure {figure_id}"
            ),
            chunk_type="figure",
            breadcrumb=breadcrumb,
            document_id=document_id,
            ...
            citation_refs=self._get_refs(block, "citation_refs"),
            footnote_refs=self._get_refs(block, "footnote_refs"),
            extra_metadata={"figure_id": figure_id, "image_ref": metadata.get("image_ref")},
        )
    ...
    def _build_equation_chunk(
        ...
        return self._make_chunk(
            ...
            text=text,
            embedding_text=self._build_embedding_text(
                text=text, breadcrumb=breadcrumb, prefix=f"Equation {equation_id}"
            ),
            chunk_type="equation",
            breadcrumb=breadcrumb,
            document_id=document_id,
            ...
            citation_refs=self._get_refs(block, "citation_refs"),
            footnote_refs=self._get_refs(block, "footnote_refs"),
            extra_metadata={"equation_id": equation_id},
        )
    ...
    def _build_fallback_chunk(
        ...
        return self._make_chunk(
            ...
            position=position,
            source_block_ids=self._single_block_id(block),
            extra_metadata={"original_block_type": block.block_type},
        )
    ...
    def _split_oversized_paragraph(
        ...
        pieces = self._prose_splitter.split_text(
            text
        )
