...
from medrag.retrieval.pgvector_store import PgConfig, PgVectorStore, EMBEDDING_DIM
...
class PgDenseIndex:
    ...
    def chunk_ids(self) -> np.ndarray:
        ...
        if self._chunk_ids_cache is None:
            ...
            cur = conn.cursor()
            cur.execute(f"SELECT chunk_id FROM medrag.embeddings ORDER BY chunk_id")
