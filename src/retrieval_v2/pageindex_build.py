...
from src.retrieval_v2.document_index import LogicalDocumentIndex, TABLE_TYPES
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # load the corpus ONCE so per-paper chunk mapping never re-reads the parquet
    corpus_df = None
    if args.from_md and not args.no_chunk_map:
        import pandas as pd
        print("Loading corpus once for chunk mapping ...")
        corpus_df = pd.read_parquet(
            Path(args.index_dir) / "corpus.parquet",
        ...
        try:
            if args.from_md:
                artifact = build_from_md(
                    ...
                    map_chunks=not args.no_chunk_map, corpus_df=corpus_df)
                built += 1
...
def build_from_md(
    ...
    from src.retrieval_v2.xml_tree import load_paper_chunks, normalize_text
    ...
    chunks = load_paper_chunks(paper_id, corpus_path, df=corpus_df) if map_chunks else {}
    structure = tree.get("structure") or []
    ...
    para_ids = [cid for cid in ordered_ids if chunks[cid]["chunk_type"] == "paragraph"]
    ...
    def chunk_score(cc: Dict[str, Any], want: str) -> float:
        ...
        if ct in want:
            return 1.8
        # cheap gates BEFORE expensive SequenceMatcher:
        la, lb = len(ct), len(want)
        if la == 0 or lb == 0:
            return 0.0
        short, long = (la, lb) if la < lb else (lb, la)
        if long / short > 1.6:
            return 0.0
        wa = set(re.findall(r"[a-z0-9]+", ct[:4000]))
        wb = set(re.findall(r"[a-z0-9]+", want[:4000]))
        if not wb:
            return 0.0
        if len(wa & wb) / len(wb) < 0.45:
            return 0.0
