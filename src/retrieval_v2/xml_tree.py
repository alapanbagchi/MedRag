...
# ---------------------------------------------------------------------------
def load_paper_chunks(paper_id: str, corpus_path: Path,
                       df=None) -> Dict[str, Dict[str, Any]]:
    ...
    them (part 10 / 23). Pass a pre-loaded full-corpus DataFrame (``df``) when
    processing many papers so the parquet is read ONCE; otherwise the read is
    pushed down to only the rows of the requested paper.
    """
    import pandas as pd
    sub = None
    if df is not None:
