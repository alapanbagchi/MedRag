"""Step 2 — Per-subquery UMLS entity enrichment.

Given a subquery's extracted entities, resolve each against UMLS/MeSH to obtain
its preferred name and synonyms, then fold those terms back into the subquery's
retrieval query so the first hybrid search already uses canonical vocabulary.

This is a plain async function — no LLM — so it can be both:
  * called deterministically during the enrich step, and
  * exposed later (Step 5) as an LLM tool the agent may invoke at will.
...
Design notes:
  ...
  * Degrades gracefully: no API key / network failure -> entities unchanged,
    subquery query left intact. Enrichment must never break retrieval.
...
logger = logging.getLogger("src.agentic.umls_tool")
...
class UMLSEnricher:
    """Resolves subquery entities against UMLS and folds terms into the query."""
...
# ---------------------------------------------------------------------------
# Tool form (Step 5 hook): a plain async callable the LLM can invoke directly.
# ---------------------------------------------------------------------------

class _UMLSEnrichContext:
    """Holds a shared UMLSEnricher so the agent tool reuses one client."""

    def __init__(self, enricher: Optional[UMLSEnricher] = None):
        self.enricher = enricher or UMLSEnricher()


async def umls_lookup_tool(entity_text: str) -> dict:
    """Tool: look up a medical term in UMLS/MeSH; returns preferred name,
    CUI, and synonyms. Used by the agent to enrich/expand a search term."""
    enricher = UMLSEnricher()
    concept = await enricher.umls.search_concept(entity_text)
    if not concept or not concept.found:
        return {"term": entity_text, "found": False, "cui": "", "preferred_name": None, "synonyms": []}
    return {
        "term": entity_text,
        "found": True,
        "cui": concept.cui,
...
if __name__ == "__main__":
    ...
    async def _run():
        from src.agentic.planner import PlannedEntity, DecomposePlanner
        ...
        sub = SubQueryPlan(
            ...
            entities=[PlannedEntity(text="radial artery vasospasm", role="condition")],
        )
        terms = await enricher.enrich_subquery(sub)
        print("terms:", [(t.preferred_name, t.synonyms) for t in terms])
        print("expanded query:", enricher.apply_to_query(sub, terms))
