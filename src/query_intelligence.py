...
from src.retrieval_v2.models import ClinicalEntity
...
def _extract_by_verb(question: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    for verb in _AFFECT_VERBS:
        ...
        if m:
            return m.group(1).strip(), m.group(2).strip()
        m = re.search(r"(?:effect|impact|influence) of\s+(.+?)\s+on\s+(.+?)(?:[,?]|$)", question, re.IGNORECASE)
        if m:
            return m.group(1).strip(), m.group(2).strip()
        m = re.search(r"role of\s+(.+?)\s+in\s+(.+?)(?:[,?]|$)", question, re.IGNORECASE)
        if m:
            return m.group(1).strip(), m.group(2).strip()
