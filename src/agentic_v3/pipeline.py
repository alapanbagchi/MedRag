...
logger = logging.getLogger("src.agentic_v3.pipeline")
...
class AgenticV3Pipeline:
    ...
    async def _answer(self, query: str) -> Dict[str, Any]:
        ...
        # Route LLM calls into the event stream for the UI (shared observer).
        # With Logfire enabled the LLM spans go to Logfire (PydanticAI's own
        # instrumentation), so the UI observer is not installed.
        observer_installed = (self.events is not None and self.events.enabled
                              and not lf.enabled())
        if observer_installed:
            from src.agentic_v2 import llm_observer
            llm_observer.install(self.events.emitter)
        try:
    ...
    async def _master_plan(self, state: V3RunState, trace: Any) -> MasterPlan:
        try:
            ...
            from src.agentic_v3.master import build_plan, fallback_task
            ...
            from src.agentic_v3.master import build_plan
    ...
    async def _resolve_contradictions(self, state: V3RunState,
        ...
        async def one(c: Contradiction) -> Contradiction:
            try:
                ...
                from src.agentic_v3.state import ResolutionOutcome
    ...
    async def _synthesize(self, state: V3RunState, trace: Any) -> Any:
        try:
            ...
            from src.agentic_v3.synthesize import SynthesisReport
