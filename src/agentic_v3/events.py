...
Reuses the thread-safe JSONL EventEmitter from src.agentic_v2.events
(import, not modification), so agentic v3 runs can be watched with the same
log-file + polling-UI pattern:
...
# Reuse the exact v2 emitter (same JSONL format, thread-safe). This is an
# import - agentic_v2 itself is left untouched.
from src.agentic_v2.events import EventEmitter, get_emitter, reset_emitter
